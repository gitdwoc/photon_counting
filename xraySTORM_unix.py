...
divide the total number of images 'tiflist' to be analysed evenly amoungst all the proccesors in the machines CPU
def multiSTORM
    if __name__ == "__main__":
        tic = time.time()
        p = Pool()
        r = p.starmap(xraySTORM, zip(tiflist, itertools.repeat(davg) itertools.repeat(10), itertools.repeat(structure)), chunksize=None )
        p.close()
        p.join()
        toc = time.time()
