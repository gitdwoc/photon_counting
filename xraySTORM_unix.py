...
This unix version uses unix path directory structure to save the output csv and 
a multiprocessing function that is compatiable with unix systems
"""
...
#given an image, a average dark current image 'davg', a thresholding value 'binn', a structuring element 'structure'
...
#to analyse large datasets the python multi processing module fucntion "pool starmap" can be used to
#divide the total number of images 'imagelist' to be analysed evenly amoungst all the proccesors in the machines CPU
def multiSTORM(imagelist, davg, binn,structure, savepath):
    if __name__ == "__main__":
        tic = time.time()
        p = Pool()
        r = p.starmap(xraySTORM, zip(imagelist, itertools.repeat(davg), itertools.repeat(10), itertools.repeat(structure)), chunksize=None )
        r = 0
